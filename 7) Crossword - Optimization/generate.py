...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap is None:
            return False
        for x_ in set(self.domains[x]):
            if not any(x_[overlap[0]] == y_[overlap[1]] for y_ in self.domains[y]):
    ...
    def ac3(self, arcs=None):
        ...
        """
        if arcs is None:
            queue = [(x, y) for x, y in self.crossword.overlaps if x != y]
        else:
            queue = list(arcs)
    ...
    def consistent(self, assignment):
        ...
        """
        # Check if all the words are different
        ...
        for key1, value1 in assignment.items():
            for key2, value2 in assignment.items():
                if key1 != key2:
                    overlap = self.crossword.overlaps[key1, key2]
                    if overlap and value1[overlap[0]] != value2[overlap[1]]:
                        return False
        return True
    ...
    def order_domain_values(self, var, assignment):
        ...
        return sorted(self.domains[var], key=lambda x: sum(any(x[self.crossword.overlaps[var, v][0]] != y[self.crossword.overlaps[var, v][1]] for y in self.domains[v]) for v in neighbors))
